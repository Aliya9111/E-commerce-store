...
@section('AuthTiltle','Password Reset Email')
...
@section('commonAuth')
    ...
    <div class="col-4 backcolor p-3">
        <h2 class="text-center text-light AuthHeads" style="font-size: 23px">Password Reset</h2> 
        <form method="post" action="{{route('emailSendSave')}}">
            @csrf
            <div class="input-group mt-4 mb-2">
                <div class="input-group-text">
                    <label for=""><i class="fa-solid fa-envelope"></i></label>
                </div>
                <input type="email" name="email" id="" class="form-control" placeholder="Enter email for reset password" style="font-size: 14px;font-family:sans-serif">
            </div>
            @error('email')
                <p class="text-danger" style="font-size: 13px;">{{$message}}</p>
            @enderror
            <div class="justify-content-center text-center">
                <button type="submit" class="btn btn-primary mt-3" style="background-color: rgb(89, 111, 219);font-weight:bold" id="sendbtn" onclick="document.getElementById('sendbtn').classList.add('disabled')">Send</button>
            </div>
        </form>  
    </div> 
