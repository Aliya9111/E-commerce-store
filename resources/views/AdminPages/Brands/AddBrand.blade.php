    ...
    <div class="row mt-3 ms-3 me-3 gy-2 justify-content-around">
        <div class="row">
            <div class="col-12 d-flex">
                <h3>Add Brand:</h3>
                <a href="{{route('Brand')}}" class="btn btn-outline-dark ms-3 btn-sm mt-1 mb-1">Clear data</a>
            ...
            <div class="row">
                <div class="col-12 bg-light shadow-sm">
                    <form method="POST" action="{{route('AddBrand')}}">
                        ...
                        <input type="text" name="bname" placeholder="Enter name of brand" class="form-control">
                        @error("bname")
                            <p class="text-danger">{{$message}}</p>
                        @enderror
                        {{--  slug of the brand  --}}
                        <label class="form-label @error('bname') mt-0 @else mt-3 @enderror"><b>Slug:</b></label>
                        <input type="text" name="bslug" placeholder="Enter the slug" class="form-control @error('bslug') mb-0 @else mb-3 @enderror">
                        @error("bslug")
                        <p class="text-danger">{{$message}}</p>
                        @enderror
                </div>
        ...
        <div class="row">
            {{--  buttons  --}}
            <div class="col-12 mb-5 @error('bslug') mt-3 @else mt-1 @enderror">
