        ...
        <div class="col-md-7 col-12 g-2">
            <div class="row">
                <div class="col-12 bg-light shadow-sm">
                    <form method="POST" action="{{route('Shipping.update',$country->id)}}">
                        @csrf
                        @method('patch')
                        {{--  name of the country  --}}
                        ...
                        @enderror
                        {{--  code of the country  --}}
                        <label class="form-label @error('cname') mt-0 @else mt-3 @enderror"><b>Country Code:</b></label>
                        <input type="text" name="code"  value="{{old('code',$country->code)}}" class="form-control mb-3 @error('bslug') mb-0 @else mb-3 @enderror">
                        @error('code')
                        <p class="text-danger">{{$message}}</p>
                        @enderror
                        ...
                        <input type="text" name="amount" value="{{old('amount',$country->amount)}}" placeholder="Enter Shipping price" class="form-control mb-3">
                        @error("amount")
                        <p class="text-danger">{{$message}}</p>
                        @enderror
                </div>
