...
@section('MainPart')
    ...
    <div class="row mt-3 ms-3 me-3 gy-2 justify-content-center">
        <div class="col-12 d-flex">
            <h3>All Countries:</h3>
            <a href="{{route('Shipping.create')}}" class="btn btn-outline-dark ms-3 btn-sm mt-1 mb-1">Add new</a>
            <a href="{{route('Shipping.index')}}" class="btn btn-outline-dark ms-3 btn-sm mt-1 mb-1">Clear</a>
        </div>
        ...
        <div class="row">
            <div class="col-md-8 col-12 mt-2">
                <form method="get" action="{{route('Shipping.index')}}">
                    ...
                    <div class="input-group mb-3">
                        <input type="search" name="Countrykeyword" value="{{Request::get('Countrykeyword')}}" class="form-control" placeholder="search" aria-label="Username" aria-describedby="basic-addon1">
            ...
            <div class="col-md-4 col-12 mt-2">
                <select class="form-select" name="countrySort" id="sortValue" onchange=ApplySort()>
                    <option value="asc" {{request('countrySort')=='asc'? 'selected':''}}>Assecding</option>
                    <option value="desc" {{request('countrySort')=='desc'? 'selected':''}}>Descssecding</option>
                    <option value="Latest" {{request('countrySort')=='Latest'? 'selected':''}}>Latest</option>
                    <option value="Oldest" {{request('countrySort')=='Oldest'? 'selected':''}}>Oldest</option>
                </select>
            </div>
            ...
            <table class="table table-striped mt-2">
                <thead align="center" style="white-space: nowrap;">
                    <tr>
                        ...
                        <th scope="col" class="borderb-Set">Amount</th>
                ...
                <tbody align="center">
                    @if($countries->isNotEmpty())
                        @foreach($countries as $country)
                            <tr  class="align-content-center">
                                ...
                                {{-- Delete--}}
                                <td>
                                    <form method="POST" action="{{route('Shipping.destroy',$country->id)}}">
                                        @csrf
                                        @method('delete')
                                        ...
                                        <a class="" href="{{route('Shipping.edit',$country->id)}}"><i class="fa-solid fa-pen text-info" ></i></a>
                                    </form>
                                    {{--  Update  --}}
                                </td>
                            </tr>
                    ...
                    @else 
                        <tr align="center">
                            <td colspan="9">No Country availabe</td>
                        </tr>
                    @endif  
...
<script>
    function ApplySort(){
        ...
        window.location.href = url + '&countrySort=' + getValue;
    }
</script>
@endsection